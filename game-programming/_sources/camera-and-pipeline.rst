...
Camera được xác định bởi điểm mắt :math:`E`, điểm ngắm :math:`T` và một hướng
lên tham chiếu :math:`\bm{u}_0`. Với hệ tọa độ tay phải và camera nhìn theo
trục :math:`-z`, đặt

.. math::

   \bm{f} = \frac{\overrightarrow{ET}}{\|\overrightarrow{ET}\|},
   \qquad
   \bm{r} = \frac{\bm{f} \times \bm{u}_0}{\|\bm{f} \times \bm{u}_0\|},
   \qquad
   \bm{u} = \bm{r} \times \bm{f}.

Ba vector :math:`\bm{r}, \bm{u},-\bm{f}` tạo thành một cơ sở trực chuẩn của camera. Nếu :math:`\bm{f}` gần song song :math:`\bm{u}_0`, tích có hướng gần :math:`0` và phải chọn một up vector khác.
...
.. math::
   ...
   M_{\mathrm{view}} =
   \begin{bmatrix}
   r_x & r_y & r_z & - \bm{r} \cdot \overrightarrow{OE}\\
   u_x & u_y & u_z & - \bm{u} \cdot \overrightarrow{OE}\\
   -f_x & - f_y & - f_z & \bm{f} \cdot \overrightarrow{OE}\\
   0 & 0 & 0 & 1
   \end{bmatrix}.
...
Từ các tam giác đồng dạng, điểm :math:`A(x_A,y_A,z_A)` trong camera space có
tọa độ ảnh tỉ lệ với

.. math:: x'_A \propto \frac{x_A}{-z_A}, \qquad y'_A \propto \frac{y_A}{-z_A}.
