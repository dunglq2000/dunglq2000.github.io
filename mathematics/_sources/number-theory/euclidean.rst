...
.. prf:theorem:: Định lí cơ bản của số học
   :label: thm-principal-arith

   Mỗi số nguyên lớn hơn :math:`1` đều có thể phân tích thành tích các số nguyên tố theo một cách duy nhất.

Vành modulo số nguyên (кольца вычётов)
======================================

Cố định :math:`m \in \NN` với :math:`m > 1`.

Hai số :math:`a, b \in \ZZ` được gọi là **đồng dư modulo** :math:`m` (hay **сравнимы по модулю** :math:`m`) nếu :math:`a` và :math:`b` có cùng số dư khi chia cho :math:`m`.

Nói cách khác, :math:`m \mid (a - b)`. Khi đó ta kí hiệu

.. math:: a \equiv b \pmod{m}.

**Lớp thương** (hay **класс вычётов**) của số nguyên :math:`k` theo modulo :math:`m` là tập

.. math:: k \bmod m = \{ l \in \ZZ : l \equiv k \bmod m \} = \{ l = k + m \cdot n : n \in \ZZ \} = k + m \ZZ.

Kí hiệu ngắn gọn là :math:`k \bmod m = \overline{k}`.

Tính chất của lớp thương:

1. Trong một lớp thương mọi số đều đồng dư với nhau theo modulo :math:`m`, tức là đều có cùng số dư khi chia cho :math:`m`.
2. Các số nguyên từ các lớp thương khác nhau thì không đồng dư theo modulo :math:`m`.
3. Các lớp thương khác nhau thì không giao nhau.
4. Mọi lớp thương modulo :math:`m` phân chia tập :math:`\ZZ` thành các tập con không giao nhau.

Tập hợp tất cả lớp thương modulo :math:`m` được kí hiệu là

.. math:: \ZZ_m = \{ \overline{0}, \overline{1}, \ldots, \overline{m-1} \}.

Các toán tử trên :math:`\ZZ_m`:

1. Phép cộng: :math:`\overline{k} + \overline{l} = \overline{k + l}`.
2. Phép nhân: :math:`\overline{k} \cdot \overline{l} = \overline{k \cdot l}`.

Lớp :math:`\overline{a} \in \ZZ_m` được gọi là **khả nghịch** (hay **обратимый**) nếu tồn tại :math:`\overline{b} \in \ZZ_m` sao cho :math:`\overline{a} \cdot \overline{b} = 1`.

Khi đó :math:`\overline{b}` được gọi là **nghịch đảo** (hay **обратый**) của :math:`\overline{a}`.

Dễ thấy, nghịch đảo, nếu tồn tại, của :math:`\overline{a}` là duy nhất. Ta có thể chứng minh bằng phản chứng.

Lớp khác không :math:`\overline{a} \in \ZZ_m` được gọi là **делитель нуля** nếu tồn tại :math:`\overline{b} \neq \overline{0} \in \ZZ_m` sao cho :math:`\overline{a} \cdot \overline{b} = \overline{0}`.

Ví dụ, :math:`\overline{2} \in \ZZ_4` có :math:`\overline{2} \cdot \overline{2} = \overline{4}` nên :math:`\overline{2}` là делитель нуля trong của :math:`\ZZ_4`.

Chú ý rằng, khi :math:`\overline{a} \cdot \overline{b} = \overline{0}` trong :math:`\ZZ_m` thì ta không thể suy ra :math:`\overline{a} = \overline{0}` hay :math:`\overline{b} = \overline{0}`.

Nếu :math:`p` là số nguyên tố thì trong :math:`\ZZ_p` không có делитель нуля.

Tập hợp tất cả lớp khả nghịch trong :math:`\ZZ_m` được kí hiệu là :math:`\ZZ_m^{\times}`.

Ta có tính chất trên :math:`\ZZ_m`:

1. Với mọi :math:`a \in \ZZ`, :math:`a \equiv a \pmod{m}`.
2. Nếu :math:`a \equiv b \pmod{m}` thì :math:`b \equiv a \pmod{m}`.
3. Nếu :math:`a \equiv b \pmod{m}` và :math:`b \equiv c \pmod{m}` thì :math:`a \equiv c \pmod{m}`.
4. Nếu :math:`a \equiv b \pmod{m}` thì :math:`a \cdot c \equiv b \cdot c \pmod{m}`.
5. Nếu :math:`c \cdot a \equiv c \cdot b \pmod{m}` và :math:`\gcd(c, m) = 1` thì :math:`a \equiv b \pmod{m}`.
6. Nếu :math:`a \equiv b \pmod{m}` thì :math:`c \cdot a \equiv c \cdot b \pmod{cm}`.
7. Nếu :math:`a \equiv b \pmod{m}` và :math:`c \equiv d \pmod{m}` thì :math:`a \pm c \equiv b \pm d \pmod{m}` và :math:`a \cdot c \equiv b \cdot d \pmod{m}`.
8. Nếu :math:`a \equiv b \pmod{m}` thì :math:`a^n \equiv b^n \pmod{m}`.
9. Nếu :math:`a \equiv b \pmod{m}` và :math:`f(x)` là đa thức với hệ số nguyên thì :math:`f(a) \equiv f(b) \pmod{m}`.
...
.. prf:corollary::
   ...
   Tồn tại các số :math:`u` và :math:`v` sao cho

   .. math:: u \cdot a + v \cdot b = \gcd(a, b).

.. prf:lemma:: 
   :label: lem-gcd

   Giả sử :math:`p` là số nguyên tố. Khi đó nếu :math:`a \cdot b` chia hết cho :math:`p` thì :math:`a` chia hết cho :math:`p` hoặc :math:`b` chia hết cho :math:`p`.

.. admonition:: Chứng minh
   :class: danger, dropdown

   Nếu :math:`a` không chia hết cho :math:`p` thì :math:`\gcd(a, p) = 1`, hay tồn tại :math:`u` và :math:`v \in \ZZ` sao cho :math:`u \cdot a + v \cdot p = 1`. Khi đó nhân hai vế cho :math:`b` ta có

   .. math:: u \cdot ab + v \cdot bp = b,

   mà :math:`p \mid ab` và :math:`p \mid p` nên :math:`p \mid b`.

Hai số :math:`a` và :math:`b` được gọi là **nguyên tố cùng nhau** (hay **coprime**, **взаимо простые**) nếu :math:`\gcd(a, b) = 1`.

Số tự nhiên :math:`p` được gọi là **số nguyên tố** (hay **prime**, **простой**) nếu nó chỉ có hai ước là :math:`1` và chính nó.
...
Cho :math:`n` là số tự nhiên lớn hơn :math:`4`. Chứng minh rằng :math:`n` là số nguyên tố khi và chỉ khi :math:`n \nmid (n - 1)!`.

----

Cho :math:`n > 1`, :math:`p \nmid n` với :math:`p` là số nguyên tố sao cho :math:`p \leqslant \sqrt{n}`. Chứng minh rằng :math:`n` là số nguyên tố.

----

Cho :math:`\gcd(a, b) = 1`. Chứng minh rằng :math:`\gcd(7a + 6b, 8a + 7b) = 1`.

Ta có :math:`\gcd(a, b) = \gcd(a, ka + b)` với :math:`k \in \ZZ`. Khi đó

.. math:: \gcd(7a + 6b, 8a + 7b) = \gcd(7a + 6b, a + b) = \gcd(a + b, a) = \gcd(b, a) = 1.

----

Giải phương trình :math:`3x^2 - 2 = y^2` trên :math:`\ZZ`.

Giải theo modulo 3.

----

Chứng minh rằng tồn tại vô hạn số nguyên tố dạng :math:`3k + 2` với :math:`k \in \ZZ`.
