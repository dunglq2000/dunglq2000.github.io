...
Tương tự, để đồng nhất và rút gọn hệ số cho hợp với bên vế trái ta chọn :math:`2 r - y_0 + c = -d` và :math:`y_0 - c = -d`. Suy ra :math:`d = -r`. Thu gọn phương trình ta được
...
Với :math:`\theta = 0` thì :math:`(x, y) = (-r, y_0)`. Với :math:`\theta = \pi / 2` thì :math:`(x, y) = (r (\pi / 2 - 1), y_0 - 2 r)`.

Tới đây ta cũng thêm bớt một hằng số vào hoành độ và tung độ để "kéo" các tọa độ về trục.

Ta đưa tọa độ khi :math:`\theta = 0` về :math:`Oy` thì :math:`x' = x + r`. Tương tự ta đưa tọa độ khi :math:`\theta = \pi / 2` về :math:`Ox` thì :math:`y' = y - y_0 + 2 r`. Khi đó tọa độ (mới) là :math:`(0, 2 r)` và :math:`(\pi r / 2, 0)`. Điều này phù hợp với yêu cầu bài toán và tương đương với phương trình trong tọa độ cực

.. math::

    \begin{cases}
        x = r (\theta - \cos \theta) + r = r (1 + \theta - \cos \theta) \\
        y = (y_0 - r) - r \sin \theta - (y_0 - 2 r) = r (1 - \sin \theta)
    \end{cases}

với :math:`0 \leqslant \theta \leqslant \dfrac{\pi}{2}.`

Đây chính là kết quả cần tìm. Thêm nữa vị trí ban đầu của vật là :math:`(0, y_0)` và tọa độ theo phương trình là :math:`(0, 2 r)` nên suy ra :math:`y_0 = 2 r`.
...
Từ bên trên và :math:`y_0 = 2 r` ta có
