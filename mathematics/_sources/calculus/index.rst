...
Để tìm cực trị của hàm :math:`f(x, y)` với hàm ràng buộc :math:`g(x, y) = 0`, ta thiết lập hàm bổ trợ, gọi là **hàm Lagrange**:

.. math:: L(x, y, \lambda) = f(x, y) + \lambda \cdot g(x, y),

trong đó :math:`\lambda` là nhân tử hằng chưa xác định, gọi là **nhân tử Lagrange**.

Điều kiện cần của cực trị là hệ phương trình

.. math:: \left\{\begin{array}{ccccc}L_x'(x, y, \lambda) & = &f_x'(x, y) + \lambda \cdot g_x'(x, y) & = & 0 \\ L_y'(x, y, \lambda) & = & f_y'(x, y) + \lambda \cdot g_y'(x, y) & = & 0 \\ L_{\lambda}'(x, y, \lambda) & = & g(x, y) & = & 0 \end{array}\right.

Giải hệ trên ta tìm được nghiệm :math:`x_0`, :math:`y_0` và :math:`\lambda_0`.

Vấn đề tồn tại và đặc tính của cực trị có điều kiện được giải bằng cách xét dấu vi phân cấp 2 của hàm Lagrange tại điểm :math:`P(x_0, y_0)` và :math:`\lambda_0`, trong đó :math:`P(x_0, y_0)` là điểm dừng của hàm :math:`L`.

Ta có

.. math:: d^2 L = L_x''\,dx^2 + 2 L_{xy}''\,dx\,dy + L_y''\,dy^2,

trong đó :math:`dx`, :math:`dy` thỏa mãn ràng buộc biểu thị bằng phương trình

.. math:: g_x'\,dx + g_y'\,dy = 0, \quad (dx^2 + dy^2 \neq 0).

Cụ thể, hàm :math:`f(x, y)` đạt cực đại có điều kiện nếu :math:`d^2 L < 0` và đạt cực tiểu nếu :math:`d^2 L > 0`, tại điểm dừng :math:`P(x_0, y_0)` và nhân tử :math:`\lambda_0`.
