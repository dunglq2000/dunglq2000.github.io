...
.. prf:definition:: Đường đi
   :label: def-graph-path

   Một dãy có thứ tự các đỉnh :math:`v_{i_0}`, :math:`v_{i_1}`, ..., :math:`v_{i_n}` sao cho :math:`e_{i_t i_{t+1}} \in E` với mọi :math:`t = 0, 1, \ldots, n-1` được gọi là **đường đi**.
   
Lúc này đường đi có :math:`n+1` đỉnh :math:`v_{i_0}`, :math:`v_{i_1}`, ..., :math:`v_{i_n}` và :math:`n` cạnh :math:`e_{i_0 i_1}`, :math:`e_{i_1 i_2}`, ..., :math:`e_{i_{n-1} i_n}`.
...
.. figure:: ../../figures/graph/subgraph-01.*

   Đồ thị ban đầu

.. _subgraph-2:

.. figure:: ../../figures/graph/subgraph-02.*
 
    Đồ thị con

.. prf:definition:: Đồ thị đầy đủ
   :label: def-graph-complete
...
.. prf:definition:: Đồ thị hai phía
   :label: def-graph-bipartite
...
.. prf:definition:: Đồ thị phẳng
   :label: def-graph-plane-graph
...
.. admonition:: Chứng minh
   ...
   Ta sử dụng quy nạp theo số cạnh :math:`e`.

   Đầu tiên, với đồ thị gồm một đỉnh thì :math:`e = 0`, :math:`v = 1` và :math:`f = 1`. Khi đó :math:`v - e + f = 2`.

   Tiếp theo, với :math:`e = 1` là một cạnh nối hai đỉnh, tức :math:`v = 2`. Lúc này một cạnh không chia mặt phẳng thành các phần khác nhau nên :math:`f = 1`, suy ra :math:`v - e + f = 2`.

   Giả sử công thức đúng với mọi :math:`e < k`, tức là :math:`v - e + f = 2`.

   Với :math:`q = k`, ta có hai trường hợp.

   Trường hợp 1. Cạnh thứ :math:`k` tạo thành một hoặc một vài chu trình. Khi đó chu trình tạo thành mặt phẳng đóng, tức là :math:`f' = f - 1`, như vậy :math:`v - (k - 1) + f' = 2`, tương đương :math:`v - k + 1 + f - 1 = 2`, ta có điều phải chứng minh.

   Trường hợp 2. Cạnh thứ :math:`k` nối hai thành phần liên thông. Giả sử thành phần liên thống thứ nhất có :math:`v_1` đỉnh, :math:`e_1` cạnh và :math:`f_1` mặt. Tương tự, thành phần liên thông thứ hai có :math:`v_2` đỉnh, :math:`e_2` cạnh và :math:`f_2` mặt. Theo giả thiết quy nạp thì

   .. math:: v_1 - e_1 + f_1 = 2, v_2 - e_2 + f_2 = 2 \Longrightarrow (v_1 + v_2) - (e_1 + e_2) + (f_1 + f_2) = 4.

   Ở đây :math:`v_1 + v_2 = v`, :math:`e_1 + e_2 = e - 1` (cạnh mới nối hai thành phần liên thông) và :math:`f_1 + f_2 = f + 1` (mặt phẳng ở ngoài tính hai lần). Như vậy :math:`v - e + f = 2`.
