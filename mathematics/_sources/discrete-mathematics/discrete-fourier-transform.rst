...
DFT và phép chia đa thức
========================

.. prf:theorem:: Chia đa thức nhanh
   :label: DFT-fast-poly-division

   Cho :math:`T` là vành giao hoán có đơn vị và chứa :math:`2^{-1}`. Cho
   :math:`F(x), G(x) \in T[x]`, với bậc không quá :math:`n \geqslant 3`,
   đồng thời hệ số cao nhất của :math:`G` khả đảo trong :math:`T`. Nếu
   ...
   thì :math:`Q` và :math:`R` có thể được tính bằng các phép nhân DFT với
   :math:`O(n \log n)` phép nhân và
   :math:`O(n \log n \log \log n)` phép cộng trong :math:`T`.

Cấu hình phép nhân DFT
----------------------

Mọi phép nhân đa thức trong thuật toán chia đều dùng cùng quy trình như
phần nhân phía trên. Với :math:`A(x)` và :math:`B(x)`, chọn :math:`N` là
lũy thừa của :math:`2` sao cho

.. math::

   N > \deg A + \deg B.

Thêm hệ số :math:`0` để hai vectơ hệ số có độ dài :math:`N`, rồi tính

.. math::

   \widehat{\bm{a}} = \operatorname{DFT}_N(\bm{a}),
   \qquad
   \widehat{\bm{b}} = \operatorname{DFT}_N(\bm{b}),

.. math::

   \widehat c_i = \widehat a_i \widehat b_i,
   \qquad
   \bm{c} = \operatorname{IDFT}_N(\widehat{\bm{c}}).

Ở đây :math:`\operatorname{DFT}_N` là biến đổi loại :math:`2` dùng các lũy
thừa lẻ của :math:`\zeta_{2N}`, đúng theo quy ước của phần nhân đa thức.
Nếu :math:`T` không chứa trực tiếp căn đơn vị cần thiết, ta dùng cấu trúc
đệ quy đã chứng minh trong :prf:ref:`DFT-thr-Tx`.

Vì :math:`N` lớn hơn bậc của tích, phép chập vòng không cuộn hệ số bậc cao
về đầu vectơ. Ta kí hiệu phép nhân này là
:math:`\operatorname{Mul}_{\mathrm{DFT}}(A, B)`.

Đảo hệ số và nghịch đảo chuỗi
-----------------------------

Đặt :math:`n = \deg F`, :math:`m = \deg G` và
:math:`\ell = n - m + 1`. Với đa thức :math:`A` bậc :math:`d`, định nghĩa

.. math::

   \operatorname{rev}_d A(x) = x^d A(x^{-1}).

Kí hiệu

.. math::

   F^* = \operatorname{rev}_n F,
   \qquad
   G^* = \operatorname{rev}_m G,
   \qquad
   Q^* = \operatorname{rev}_{n-m} Q.

Đảo hệ số trong :math:`F = QG + R` rồi lấy modulo :math:`x^\ell` cho

.. math::

   F^* \equiv Q^* G^* \pmod{x^\ell},

do đó

.. math::

   Q^* \equiv F^* (G^*)^{-1} \pmod{x^\ell}.

Số hạng tự do của :math:`G^*` là hệ số cao nhất của :math:`G`, nên nghịch
đảo chuỗi tồn tại. Nếu :math:`H_k G^* \equiv 1 \pmod{x^k}`, phép lặp

.. math::

   H_{2k}
   = H_k \left(2 - G^* H_k\right)
   \pmod{x^{2k}}

tăng gấp đôi số hệ số đúng. Hai tích ở mỗi vòng được thực hiện bằng
:math:`\operatorname{Mul}_{\mathrm{DFT}}` với độ dài biến đổi vừa đủ cho
:math:`2k` hệ số, thay vì dùng một DFT lớn cố định từ đầu.
...
.. code-block:: text
   ...
   POLY-DIV-DFT(F, G):
       ...
       length <- n - m + 1
       Fr <- reverse(F, n) mod x^length
       Gr <- reverse(G, m) mod x^length

       H <- inverse(leading-coefficient(G))
       k <- 1
       while k < length:
           size <- min(2*k, length)
           H <- MulDFT(H, 2 - MulDFT(Gr, H)) mod x^size
           k <- size

       Qr <- MulDFT(Fr, H) mod x^length
       Q <- reverse(Qr, n - m)
       R <- F - MulDFT(Q, G)
       return (Q, R)

Như vậy phép chia gọi phép nhân DFT ở ba vị trí: hai tích trong mỗi bước
Newton, tích :math:`F^* (G^*)^{-1}` để tìm thương, và tích :math:`QG` để
khôi phục phần dư.

Chứng minh và đánh giá độ phức tạp
----------------------------------

Giả sử :math:`n = \deg F(x)`, :math:`m = \deg G(x)` với :math:`m \leqslant n`. Đặt :math:`F(x) = \sum\limits_{i=0}^n f_i x^i`, :math:`G(x) = \sum\limits_{j=0}^m g_j x^j`. Ta cần tìm các phần tử :math:`q_0`, ..., :math:`q_{n-m}`, :math:`r_0`, ..., :math:`r_{m-1} \in T` sao cho

.. math::
   :label: eq-12

   \sum\limits_{i=0}^n f_i x^i = \sum\limits_{i=0}^{n-m} q_i x^i \sum\limits_{j=0}^m g_j x^j + \sum\limits_{i=0}^{m-1} r_i x^i.

Thay :math:`x` bởi :math:`x^{-1}` rồi nhân cho :math:`x^n` hai vế thì :eq:`eq-12` trở thành

.. math::
   :label: eq-13

   \sum_{i=0}^n f_i x^{n-i} \equiv \sum_{i=0}^{n-m} q_i x^{n-m-i} \cdot \sum_{j=0}^m g_j x^{m-j} \bmod x^{n-m+1}.

Kí hiệu :math:`T[[x]]` là chuỗi lũy thừa hình thức (формальные степенные ряды) có dạng

.. math:: a_0 + a_1 x + a_2 x^2 + \cdots

với hệ số trong :math:`T`.

Đặt :math:`H = H(x) \in T[[x]]` là chuỗi lũy thừa nghịch đảo với :math:`G^*(x) = \sum\limits_{i=0}^m g_i x^{m-i}`. Chuỗi :math:`H` sẽ tồn tại vì theo điều kiện của định lí, phần tử :math:`g_m` khả đảo trong :math:`T`. Khi đó, tìm :math:`H \pmod{x^{N_1}}` với :math:`N_1 \in \mathbb{N}` nào đó và nhân với phương trình :eq:`eq-13` thì ta tính được :math:`Q(x) \pmod{x^{N_1}}`.

Với phần tử :math:`P \in T[[x]]` ta kí hiệu :math:`f(P) = \dfrac{1}{P} - G^* \in T[[x]]`, nếu phần tử :math:`\dfrac{1}{P} \in T[[x]]` xác định.

Rõ ràng :math:`f(H) = 0`. Với :math:`P \in T[[x]]` đặt

.. math::
   :label: eq-14

   \phi(P) = 2P - P^2 G^* = H - G^* (P - H)^2 \in T[[x]].

Giả sử :math:`P \equiv H \pmod{x^k}` với :math:`k` nào đó thuộc :math:`\mathbb{N}`.

Khi đó phương trình :eq:`eq-14` suy ra :math:`\phi(P) \equiv H \pmod{x^{2k}}`.

Nếu cho trước phần tử :math:`P` sao cho :math:`P \equiv H \pmod{x^k}` thì :math:`\phi(P) \pmod{x^{2k}}` có thể tính theo cách sau: tối giản chuỗi :math:`G^*` thành :math:`x^{2k}`; phần tử nhận được :math:`2 + (-P) G^* \pmod{x^{2k}}` ta nhân với :math:`P` và cắt bỏ chuỗi cho :math:`x^{2k}`. Ta nhận được quan hệ

.. math:: \phi(P) \pmod{x^{2k}} \equiv P (2 - P G^*) \pmod{x^{2k}} \equiv H \pmod{x^{2k}}.

Như vậy, nếu :math:`P \equiv H \pmod{x^k}` thì :math:`\phi(P) \equiv H \pmod{x^{2k}}`, và ta có thể tìm :math:`\phi(P) \pmod{x^{2k}}` với hai phép nhân trong :math:`T[x]` các đa thức với bậc nhỏ hơn :math:`2k`, và một phép cộng trong :math:`T` (cộng cho phần tử :math:`2`).

Ta bắt đầu vòng lặp với :math:`P = g^{-1} \in T` với :math:`g = g_m` là hệ số cao nhất của :math:`G(x)` đã biết.

Khi đó :math:`P \equiv H \pmod{x}`. Sau đó ta tính :math:`\phi(P) \pmod{x^2}`, :math:`\phi(\phi(x)) \pmod{x^4}`, ..., :math:`\phi^j(P) \pmod{x^{2^j}}`, ... với :math:`\phi^j` là hợp của :math:`j` phép :math:`\phi`. Khi đó :math:`\phi^j(P) \equiv H \pmod{x^{2^j}}`, và, ngoài ra, :math:`\phi^j(P) \in T[[x]]` sẽ khả đảo.

Đặt :math:`A^*(k)` và :math:`M^*(k)` là số phép cộng và nhân trong :math:`T` cần để tính :math:`H \pmod{x^k}`. Ta sẽ chứng minh

.. math::
   :label: eq-15

   A^*(2k) \leqslant A^*(k) + 2 A(2k) + 1
...
.. math::
   ...
   M^*(2k) \leqslant M^*(k) + 2 M(2k)

với :math:`A(k)` và :math:`M(k)` là số phép cộng và nhân để nhân hai đa thức (chương 9.3).

Ở đây :math:`A^*(1) = M^*(1) = 0` vì :math:`g^{-1} \in T` đã biết.

Từ chương 9.3 ta biết :math:`M(n) = O(n \log n)` nên có thể nói :math:`M(n) = C_M \cdot n \log n` với :math:`n > 1`, :math:`M(1) = C_M` với :math:`C_M` là hằng số tuyệt đối nào đó.

Tương tự có thể nói :math:`A(n) = C_A n \log n \log \log n`.

Khi :math:`n \geqslant 4` và :math:`A(n) = C_A` khi :math:`n = 1`, :math:`2`, :math:`3` với :math:`C_A` là hằng số tuyệt đối nào đó.

Khi đó :math:`M(x)`, :math:`A(x)`, :math:`\dfrac{M(x)}{x}`, :math:`\dfrac{A(x)}{x}` là các hàm **растущие**.

Từ :eq:`eq-16` bằng quy nạp suy ra

.. math::
   :label: eq-17

   M^*(2^t) \leqslant 4 M(2^t).

Tiép theo, từ :eq:`eq-15` suy ra

.. math::
   :label: eq-18

   A^*(2^t) \leqslant 6 A(2^t).

Tương tự

.. math:: A^*(2^t) \leqslant A^*(2^{t-1}) + 2 A(2^t) + 1 \leqslant \cdots \\ \leqslant A^*(1) + 2 (A(2) + A(4) + \cdots + A(2^t)) + t \\ = t + 2 \cdot \dfrac{A(2^t)}{2^t} \sum_{j=1}^t \dfrac{A(2^j) \cdot 2^t}{A(2^t)} \\ = t + 2 \cdot \dfrac{A(2^t)}{2^t} \sum_{j=1}^t 2^j \cdot \dfrac{A(2^j) / 2^j}{A(2^t) / 2^t} \\ \leqslant t + 2^{1-t} A(2^t) \sum_{j=1}^t 2^j.

Bất đẳng thức cuối đúng vì :math:`A(2^j) / 2^j` là hàm tăng khi :math:`j` tăng. Từ đây

.. math:: A^*(2^t) \leqslant t + 2 A(2^t) \cdot \dfrac{2^{t+1} - 1}{2^t} \leqslant t + 4 A(2^t) \leqslant 6 A(2^t)

vì :math:`t \leqslant 2 A(2^t)`.

Chọn :math:`t \in \mathbb{N}` nhỏ nhất sao cho :math:`2^t \geqslant n - m + 1`.

Khi đó :math:`2^t \leqslant 2 (n-m)` nếu :math:`n > m` (khi :math:`n = m` thì việc tính :math:`H \pmod{x^{n-m+1}}` là tầm thường).

Việc tính :math:`H \pmod{x^{n-m+1}}` được thực hiện không hơn :math:`A^*(2^t)` phép cộng và :math:`M^*(2^t)` phép nhân trong :math:`T`. Từ :eq:`eq-17` và :eq:`eq-18` suy ra ta cộng thêm không hơn :math:`6 A(2(n-m))` phép cộng và :math:`4 M(2(n-m))` phép nhân trong :math:`T`.

Bây giờ tính

.. math:: H \pmod{x^{n-m+1}} \cdot \left(\sum_{i=1}^n f_i x^{n-i} \pmod{x^{n-m+1}}\right)

và triệt tiêu chuỗi cho :math:`x^{n-m+1}`. Từ :eq:`eq-13` ta tính :math:`\sum\limits_{i=0}^{n-m} q_i x^{n-m-i}`. Như vậy ta đã xác định hệ số phần dư của :eq:`eq-12`. Tiếp theo ta tính tích

.. math:: \left(\sum_{i=0}^{n-m} q_i x^i\right) \cdot \left(\sum_{j=0}^{m} g_j x^j\right)

là một phép nhân đa thức bậc :math:`n-m` cho bậc :math:`m` trong :math:`T[x]`, sau đó xác định :math:`\sum\limits_{i=0}^{m-1} r_i x^i` với một phép trừ các đa thức bậc không lớn hơn :math:`n`.

Ta đánh giá số lượng phép cộng và nhân. Số phép cộng sẽ không lớn hơn

.. math::
   :label: eq-19

   6 A(2(n-m)) + A(n-m) + A(\max(m, n-m)) + n.

Số phép nhân không lớn hơn

.. math::
   :label: eq-20

   4 M(2(n-m)) + M(n-m) + M(\max(m, n-m)).

Đại lượng :eq:`eq-19` không vượt quá

.. math:: 6 A(2n) + A(n) + A(n) + n = O(n \log n \log\log n)

và đại lượng :eq:`eq-20` không vượt quá

.. math:: 4 M(2n) + 2 M(n) = O(n \log n).
