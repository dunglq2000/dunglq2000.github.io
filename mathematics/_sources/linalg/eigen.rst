...
.. prf:example::
   ...
   Trị riêng của ma trận :math:`\begin{pmatrix} 3 & 4 \\ 4 & -3 \end{pmatrix} \begin{pmatrix} x \\ y \end{pmatrix}` là :math:`\lambda_1 = 5` và :math:`\lambda_2 = -5`, lần lượt tương ứng với các vector riêng :math:`\bm{v}_1 = (2, 1)` và :math:`\bm{v}_2 = (1, -2)`.
...
.. admonition:: Chứng minh
   ...
   nên :math:`\det(\bm{A} - \lambda \bm{I}) = \lambda^2 - 25 = 0`, suy ra :math:`\lambda = \pm 5`.
   ...
   nên :math:`(2, 1)` là vector riêng ứng với trị riêng :math:`\lambda = 5`.
   ...
   nên :math:`(1, -2)` là vector riêng ứng với trị riêng :math:`\lambda = -5`.
...
.. prf:property:: Một số tính chất của trị riêng và vector riêng
    ...
    1. :math:`\tr \bm{A} = \lambda_1 + \lambda_2 + \ldots + \lambda_n`.
...
.. prf:property:: Tính chất liên quan đến rank và trace
    ...
    1. :math:`\tr (\bm{A} \bm{B}) = \tr (\bm{B} \bm{A})`.
    2. :math:`\rank (\bm{A} \bm{B}) \leqslant \min(\rank(\bm{A}), \rank(\bm{B}))`.
...
Các cột của :math:`A` có dạng :math:`\bm{v} \cdot \bm{v}_1`, :math:`\bm{v} \cdot \bm{v}_2`, ..., :math:`\bm{v} \cdot \bm{v}_n`. Như vậy các cột đều tỉ lệ với cột đầu nên :math:`\rank \bm{A} = 1`.
...
**Bài 3.** Cho ma trận :math:`\bm{A}_{3 \times 3}`. Biết rằng :math:`\tr \bm{A} = \tr \bm{A}^{-1} = 0` và :math:`\det \bm{A} = 1`. Chứng minh rằng :math:`\bm{A}^3 = \bm{I}`.

Phương trình đặc trưng có dạng :math:`P_3(\lambda) = -\lambda^3 + a_2 \lambda^2 + a_1 \lambda + a_0`.

Theo tính chất trên thì :math:`a_2 = \sum \lambda = \tr \bm{A} = 0`.
...
Nghĩa là :math:`\dfrac{1}{\lambda}` là trị riêng của ma trận :math:`\bm{A}^{-1}`. Suy ra :math:`\dfrac{1}{\lambda_1} + \dfrac{1}{\lambda_2} + \dfrac{1}{\lambda_3} = \tr A^{-1} = 0`.
...
Ta thấy rằng với :math:`k = 1` thì :math:`\lambda_1 + \ldots + \lambda_n = \tr(\bm{A}) \geqslant 0`.
...
Do đó :math:`\lambda_1^k + \ldots + \lambda_n^k = \tr(\bm{A}^k) \geqslant 0`.

----

**Bài 5.** Cho ma trận :math:`\bm{A}` khả nghịch. :math:`\bm{X}` là ma trận sao cho :math:`\bm{A} \bm{X} + \bm{X} \bm{A} = \bm{0}`. Chứng minh rằng :math:`\tr \ \bm{X} = 0`.

Nhân bên trái hai vế cho :math:`\bm{A}^{-1}` ta có :math:`\bm{X} + \bm{A}^{-1} \bm{X} \bm{A} = \bm{0}`. Ta biết rằng :math:`\bm{A}^{-1} \bm{X} \bm{A}` là ma trận tương đương ma trận :math:`\bm{X}` nên :math:`\tr (\bm{A}^{-1} \bm{X} \bm{A}) = \tr \ \bm{X}`.

Suy ra :math:`\tr \bm{X} + \tr \bm{X} = \tr \ \bm{0} = 0`. Từ đây có :math:`\tr \ \bm{X} = 0`.