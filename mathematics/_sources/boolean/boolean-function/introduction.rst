...
Theo cách hiểu cơ bản, Boolean gồm hai giá trị :math:`0` hoặc :math:`1` (sai hoặc đúng). Chương này tham khảo chính từ :cite:`Tokareva1,Tokareva2,Pank14`.
...
+------------------------+-------------------------------------------------------------+
| :math:`[n]`            | Tập chỉ số :math:`\{1, \ldots, n\}`                         |
+------------------------+-------------------------------------------------------------+
| :math:`\mathfrak{S}_n` | Tập các hoán vị trên :math:`[n]`                            |
+------------------------+-------------------------------------------------------------+

Trong toàn bộ chương này, ta dùng quy ước

.. math:: [n] = \{1, \ldots, n\}.

Hàm Boolean :math:`f` đối với các biến :math:`x_1`, :math:`x_2`, ..., :math:`x_n` nhận đầu vào trong :math:`\mathbb{F}_2^n` và trả về giá trị thuộc :math:`\mathbb{F}_2`. Nói cách khác :math:`f` là ánh xạ từ :math:`\mathbb{F}_2^n` tới :math:`\mathbb{F}_2`.

.. prf:definition:: Support
   :label: def-support-boolean

   Với :math:`\bm{x} = (x_1, \ldots, x_n) \in \mathbb{F}_2^n`, **support** của :math:`\bm{x}` là

   .. math:: \operatorname{supp}(\bm{x}) = \{i \in [n] : x_i \neq 0\}.

   Với hàm Boolean :math:`f : \mathbb{F}_2^n \to \mathbb{F}_2`, support của :math:`f` là

   .. math:: \operatorname{supp}(f) = \{\bm{x} \in \mathbb{F}_2^n : f(\bm{x}) \neq 0\}.

   Do đó :math:`\mathrm{wt}(\bm{x}) = |\operatorname{supp}(\bm{x})|` và :math:`\mathrm{wt}(f) = |\operatorname{supp}(f)|`.

Ta kí hiệu hàm Boolean :math:`n` biến là :math:`f(x_1, x_2, \ldots, x_n)`.

Do :math:`x_i \in \mathbb{F}_2` nên ta có :math:`2^n` vector (bộ số) :math:`(x_1, x_2, \ldots, x_n)`. Giá trị của hàm :math:`f` lại nằm trong tập :math:`\{ 0, 1 \}` nên ứng với mỗi vector có thể có :math:`2` giá trị của hàm Boolean, và ta có :math:`2^n` vector nên số lượng hàm Boolean có thể có là :math:`2^{2^n}`. Như vậy :math:`\lvert \mathcal{F}_n \rvert = 2^{2^n}`.
...
Đặt :math:`f(\bm{x})` là hàm Boolean :math:`n` biến. Với mỗi :math:`m \leqslant n` thì
...
.. admonition:: Chứng minh
   ...
   Chọn bộ :math:`(b_1, \ldots, b_m)` bất kì thuộc :math:`\mathbb{F}_2^m`. Khi thay :math:`x_i` bởi :math:`b_i` với :math:`i \in [m]` ta được
   ...
   Ở vế phải, tích :math:`\prod\limits_{i \in [m]} (b_i \oplus a_i \oplus 1) = 1` khi và chỉ khi :math:`b_i \oplus a_i \oplus 1 = 1` với mọi :math:`i \in [m]`.
...
Khi đó, :math:`f(a_1, \ldots, a_m, x_{m+1}, \ldots, x_m)` được gọi là **hệ số khai triển của hàm** :math:`f` **theo các biến** :math:`x_1`, ..., :math:`x_m`. Tương ứng với :math:`m` biến ta có :math:`2^m` hệ số khai triển.
...
.. math:: 
   ...
   \mathrm{wt}(f) = \sum_{i \in [2^m]} \mathrm{wt} (f_i).

.. prf:definition:: Algebraic normal form
   :label: def-bool-ANF

   Với hàm Boolean :math:`n` biến :math:`f(x_1, x_2, \ldots, x_n)`, **algrebaric normal form** (hay **ANF**) tương ứng với hàm bool đó là cách biểu diễn đa thức đó dưới dạng tổng các tích như sau

   .. math:: 

      f(x_1, x_2, \ldots, x_n) = \bigoplus_{I \subseteq [n]} a_I \prod_{i \in I} x_i.

   với :math:`a_I \in \FF_2`.
   
Ta thấy rằng hàm Boolean :math:`n` biến, do đó có :math:`2^n` hệ số :math:`a_i` với mỗi :math:`i = 0, 1, \ldots, 2^n-1`.
...
.. prf:definition:: Hàm Boolean cân bằng
   ...
   Nếu hàm Boolean :math:`n` biến :math:`f` có trọng số bằng :math:`2^{n-1}` thì :math:`f` được gọi là hàm Boolean **cân bằng**.
...
Do :math:`x_i^{a_i} = 1` khi và chỉ khi :math:`a_i \leqslant x_i`, ta có :math:`\bm{x}^{\bm{a}} = 1` khi và chỉ khi :math:`\bm{a} \preccurlyeq \bm{x}` theo nghĩa :math:`a_i \leqslant x_i` với mọi :math:`i \in [n]`.

Ta có thể viết lại :eq:`g` là

.. math:: f(\bm{x}) = \bigoplus_{\bm{a} \in \mathbb{F}_2^n} g(\bm{a}) \cdot \bm{x}^{\bm{a}}.
...
.. admonition:: Chứng minh
   ...
   với :math:`i \in [2^{n-d}]`. Ta có điều phải chứng minh.
...
.. prf:definition:: 
   ...
   Hàm :math:`f(x_1, \ldots, x_n)` được gọi là **linear dependent** (hay **линейно зависеть**) vào biến :math:`x_i` nếu :math:`f` có thể biểu diễn ở dạng
...
.. prf:definition:: 
   ...
   Hàm :math:`f(x_1, \ldots, x_n)` được gọi là **quasi-linear dependent** (hay **квазилинейно зависеть**) trên cặp biến :math:`x_i` và :math:`x_j` nếu :math:`f` đổi giá trị khi ta đảo giá trị ở vị trí :math:`i` và :math:`j`.
...
.. figure:: ../../figures/boolean/zhegalkin-01.*
...
.. figure:: ../../figures/boolean/zhegalkin-02.*
...
.. figure:: ../../figures/boolean/zhegalkin-03.*

   Bước 1

.. figure:: ../../figures/boolean/zhegalkin-04.*
...
.. prf:definition:: Hàm Boolean affine
   ...
   Hàm Boolean :math:`f` được gọi là **affine** nếu nó có dạng
   ...
   Nếu :math:`a_0=0` thì :math:`f` được gọi là **tuyến tính**.

Nói cách khác, hàm affine là hàm có bậc đại số không quá :math:`1`; các hàm affine khác hằng có bậc đúng bằng :math:`1`.

.. prf:example::
   :label: exp-bool-affine-linear

   Hàm :math:`f(x,y)=x\oplus y` vừa affine vừa tuyến tính, còn :math:`g(x,y)=x\oplus xy` không affine.

Vì một hàm affine được xác định bởi :math:`n+1` hệ số trong :math:`\mathbb{F}_2`, còn một hàm tuyến tính được xác định bởi :math:`n` hệ số, nên
...
.. prf:definition:: Đạo hàm Boolean
   ...
   Với :math:`\bm{a}\in\mathbb{F}_2^n`, đạo hàm của :math:`f` theo hướng :math:`\bm{a}` là

   .. math:: D_{\bm{a}}f(\bm{x})=f(\bm{x}+\bm{a})\oplus f(\bm{x}).

Trong đặc số :math:`2`, :math:`D_{\bm{a}}D_{\bm{a}}f=0`. Nếu :math:`f` không hằng và đạo hàm khác :math:`0` thì

.. math:: \deg(D_{\bm{a}}f)\leqslant\deg(f)-1.

Luôn tồn tại ít nhất một hướng đạt dấu bằng: nếu một đơn thức bậc cao nhất chứa :math:`x_i`, có thể lấy hướng là vector đơn vị :math:`\bm{e}_i`.

.. prf:definition:: Đạo hàm cấp :math:`k`
   :label: def-higher-order-boolean-derivative

   Với :math:`\bm{a}_1,\ldots,\bm{a}_k\in\mathbb{F}_2^n`, đạo hàm cấp :math:`k` là

   .. math:: D_{\bm{a}_1}\cdots D_{\bm{a}_k}f.

Nếu các hướng độc lập tuyến tính và :math:`E=\operatorname{span}(\bm{a}_1,\ldots,\bm{a}_k)` thì
...
Nếu các hướng phụ thuộc tuyến tính thì đạo hàm lặp bằng :math:`0`. Nếu đạo hàm cấp :math:`k` khác :math:`0` thì

.. math::

   \deg(D_{\bm{a}_1}\cdots D_{\bm{a}_k}f)
   \leqslant\deg(f)-k.

Một **affine flat** chiều :math:`k` là tập :math:`A=\bm{a}+E`, với :math:`E` là không gian con tuyến tính chiều :math:`k`.

.. prf:property:: Tiêu chuẩn affine flat cho bậc đại số
   :label: prop-degree-affine-flat

   Hàm :math:`f` có bậc không quá :math:`d` khi và chỉ khi restriction của nó trên mọi affine flat chiều :math:`d+1` có trọng số chẵn, tức

   .. math:: \bigoplus_{\bm{x}\in A}f(\bm{x})=0.

Để :math:`\deg(f)=d`, ngoài điều kiện trên phải tồn tại một affine flat chiều :math:`d` mà restriction của :math:`f` trên đó có trọng số lẻ.

Nếu :math:`\mu(f)` là biến đổi Möbius nhị phân của :math:`f\neq0`, bất đẳng thức Pieprzyk--Zhang cho
