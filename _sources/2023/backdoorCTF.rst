...
.. math:: x_R = x_P \cdot x_Q + D \cdot y_P \cdot y_Q \bmod n
...
Bài này tương tự bài trên và cũng có một mạnh thường quân nào đó đã donate factor của :math:`n`.
...
Bài này thuộc dạng discrete logarithm. Với hai điểm :math:`P = (x_P, y_P)` và :math:`Q = (x_Q, y_Q)`, kết quả phép cộng :math:`R = P + Q` là

.. math:: x_R = a^{-1} \cdot (x_P \cdot x_Q - y_P \cdot y_Q \cdot a^2 \cdot b^{-2}) \bmod p
...
.. math:: a^{k-1} \cdot \begin{pmatrix} x_1 & -y_1 \cdot a^2 \cdot b^{-2} \\ y_1 & x_1 \end{pmatrix}^k = \begin{pmatrix} x_2 & -y_2 \cdot a^2 \cdot b^{-2} \\ y_2 & x_2 \end{pmatrix}

Đặt :math:`M = a \cdot \begin{pmatrix} x_1 & -y_1 \cdot a^2 \cdot b^{-2} \\ y_1 & x_1 \end{pmatrix}`,
...
Alice chọn hai số bí mật :math:`n`, :math:`m` và tính :math:`a^n \cdot w \cdot b^m`.

Bob chọn hai số bí mật :math:`r`, :math:`s` và tính :math:`a^r \cdot w \cdot b^s`.

Khóa chung là :math:`a^{n + r} \cdot w \cdot b^{m + s}` và được dùng cho AES.
